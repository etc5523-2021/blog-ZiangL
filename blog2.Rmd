...
From the figure below, we can clearly observe the situation of COVID19 in various countries as of August 30th 2021. In order to better compare Kuwait, we use a separate color to distinguish. 
...
Due to the limitations of the data, only the following six dates have recorded the vaccination information. Hence, the following plot compares the number of people vaccinated during the five time periods. 
...
In order to make the data more comparable, we divided the daily new cases changing trend into 6 time periods according to the number of vaccinations. It is not difficult to see from the figure below that although the number of confirmed cases has risen sharply after the start of vaccination, after period 4, the vaccination ratio exceeds 60%, and the number of daily new cases has dropped rapidly. So we can infer that vaccination is effective in reducing the number of confirmed COVID-19

Similarly, we can see from the figure that the mean and median of positive rate drops significantly in periods 5 and 6.
...
vac_cases <- plot_ly(type = 'scatter', mode = 'lines')%>%
  ...
  add_trace(x = ~p6$date, y = ~p6$new_cases, 
            ...
            hovertemplate = paste('<b>Period6</b> (after %vax is 62%)',
                                  '<br><i>Date</i>: %{x}',
                                  '<br><i>Daily new cases</i>: %{y}<br>'))%>%
  layout(showlegend = F, title='<br>Comfirmed Cases under 6 Vaccinated Status<br> ', plot_bgcolor='#fef9ef',paper_bgcolor='#fef9ef',
         xaxis = list(title="Date"),
